...
bool SerialPort::ReadLine(std::string &data)
{
    ...
    res = read(fileDesc_, &c, 1);
    // Error Handling
    if (res < 0)
        throw std::system_error(EFAULT, std::system_category()); // Error
    if (res == 0)
        return false; // Timeout

    while (c != '\n')
    {
        readBuffer_[bytes_read] = c;
        bytes_read++;
    ...
    data = std::string(&readBuffer_[0], bytes_read);
    return true;
}
